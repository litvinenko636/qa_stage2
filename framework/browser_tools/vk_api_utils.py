...
class VKApiUtils:
    ...
    def edit_post(self, photo_info, message, post_id):
        ...
        params = 'message=' + message + self.param_divider + 'attachments=' + photo + self.param_divider + 'post_id=' + str(
            post_id)
        Logger(__name__).write_info('text /' + message + '/ has been sent')
    ...
    def save_wall_image(self):
        ...
        params = 'server=' + str(response['server']) + self.param_divider + 'photo=' + response[
    ...
    def is_post_liked(self, post_id):
        params = 'item_id=' + str(post_id) + self.param_divider + 'type=post'
        url = self.basic_url + Methods.is_liked.value + self.method_divider + params + self.access_token_and_version
        response = requests.post(url)
        Logger(__name__).write_info(response.json())
        if response.json()['response']['liked'] == 1:
            Logger(__name__).write_info('post is liked')
        else:
            Logger(__name__).write_info('post isnt liked')
        return response.json()